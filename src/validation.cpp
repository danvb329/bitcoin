...
bool CheckSequenceLocks(const CTransaction &tx, int flags, LockPoints* lp, bool useExistingLockPoints)
{
    ...
    if (useExistingLockPoints) {
        ...
        for (size_t txinIndex = 0; txinIndex < tx.vin.size(); txinIndex++) {
            const CTxIn& txin = tx.vin[txinIndex];
            CCoins coins;
            if (!viewMemPool.GetCoins(txin.prevout, coins)) {
...
bool GetTransaction(const uint256 &hash, CTransactionRef &txOut, const Consensus::Params& consensusParams, uint256 &hashBlock, bool fAllowSlow)
{
    ...
    if (fAllowSlow) { // use coin database to locate block that contains transaction, and scan it
        ...
        while (nHeight == -1 && out < 1024) { // Only try first 1024 outputs
            ...
            const CCoins* coins = view.AccessCoins(COutPoint(hash, out));
            if (coins)
...
static void ReplayBlock(const CBlock& block, CCoinsViewCache& inputs, int nHeight)
{
    for (const CTransactionRef& tx : block.vtx) {
        ...
        // We must set potential_overwrite here, as we don't know that the entry
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            const CCoins *coins = inputs.AccessCoins(prevout);
            assert(coins);
            assert(!coins->IsPruned());
            //LogPrintf("CheckTxInputs: tx %s:in%i spends from %s:out%i with coinbase=%i value=%i height=%i scriptPubKey=%s\n", tx.GetHash().GetHex(), i, prevout.hash.GetHex(), prevout.n, coins->fCoinBase, coins->out.nValue, coins->nHeight, HexStr(coins->out.scriptPubKey.begin(), coins->out.scriptPubKey.end()));
...
bool ApplyTxInUndo(CCoins&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        while (iter.n * ::GetSerializeSize(CTxOut(), SER_NETWORK, PROTOCOL_VERSION) < MAX_BLOCK_BASE_SIZE) {
            const auto alternate = view.AccessCoins(iter);
            if (alternate) {
        ...
        if (!found) {
            fClean = fClean && error("%s: undo transaction metadata missing", __func__);
        }
    }
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            for (size_t j = 0; j < tx.vin.size(); j++) {
                prevheights[j] = view.AccessCoins(tx.vin[j].prevout)->nHeight;
            }
